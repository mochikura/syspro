...
func main() {
	...
	for {
		...
		} else if rdflag == 1 { //リダイレクト時
			comnum = rdcomgo(com[:(rdnum-1)], com[rdnum+1])
		} else if rdflag == 2 { //パイプ時
...
func rdcomgo(com []string, f string) int {
	...
	cmdpath, err := exec.LookPath(com[0])
	if err != nil {
		fmt.Println(com[0] + ": No such file or directory")
		return 1
	} else {
...
func pipecomgo(com1 []string, com2 []string) int {
	...
	cmdpath1, err := exec.LookPath(com1[0])
	if err != nil {
	...
	_, err = syscall.ForkExec(cmdpath1, com1, &attr1)
	if err != nil {
		fmt.Println(err)
	}
	pout.Close()
	// execute the 2nd command
	cmdpath2, err := exec.LookPath(com2[0])
	if err != nil {
	...
	pid, err := syscall.ForkExec(cmdpath2, com2, &attr2)
	if err != nil {
		fmt.Println(err)
	}
	pin.Close()
